...
class PairwiseComBATDataFrame:
    ...
    site_id_col: None | str
    data_cols: None | list[str]
    covariate_cols: None | list[str]
    is_fitted_: bool
    ...
    def _validate_dataframe(
        ...
        if self.model.source_site not in site_values:
            raise ValueError(
                f"Source site '{self.model.source_site}' not found in {site_id_col} column. Available: {site_values}"
            )
        if self.model.target_site not in site_values:
            raise ValueError(
                f"Target site '{self.model.target_site}' not found in {site_id_col} column. Available: {site_values}"
            )
        ...
        if non_numeric_cols:
            raise ValueError(
                f"Non-numeric columns found: {non_numeric_cols}. All data and covariate columns must be numeric."
            )
        ...
        if source_count == 0:
            raise ValueError(
                f"No data found for source site '{self.model.source_site}'"
            )
        if target_count == 0:
            raise ValueError(
                f"No data found for target site '{self.model.target_site}'"
            )
    ...
    def _prepare_arrays(
        ...
        covars_ref = (
            df_ref.select(covariate_cols).to_numpy().T
        )  # Shape: (n_covariates, n_samples)

        # Extract moving site data
        Y_moving = (
            df_moving.select(data_cols).to_numpy().T
        )  # Shape: (n_locations, n_samples)
        covars_moving = (
            df_moving.select(covariate_cols).to_numpy().T
        )  # Shape: (n_covariates, n_samples)
    ...
    def transform(
        ...
        if not self.is_fitted_:
            raise ValueError(
                "Model must be fitted before transformation. Call fit() first."
            )
        ...
        if not all([site_id_col, data_cols, covariate_cols]):
            raise ValueError(
                "Column names must be provided either during fit() or transform()"
            )
        ...
        if source_data.height == 0:
            raise ValueError(
                f"No data found for source site '{self.model.source_site}' in transform data"
            )
        ...
        Y_harmonized = self.model.predict(
            covars_moving=covars_moving, Y_moving=Y_moving
        )
    ...
    def load_model(filepath: str) -> "PairwiseComBATDataFrame":
        ...
        with h5py.File(filepath, "r") as f:
            ...
            if covariate_cols is not None:
                ...
                combat.covariate_cols = (
                    covariate_cols.split(",") if covariate_cols else None
                )
            else:
