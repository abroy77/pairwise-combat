...
def site_fixture_1d_scalar(feature_property_fixture_scalar):
    ...
    return Site(
        gamma=gamma,
        delta=delta,
        feature_properties=feature_property_fixture_scalar
    )
...
class TestDataGeneration:
    ...
    def test_site_initialization_2D_feature(self,feature_property_fixture_2D):
        ...
        assert site.feature_properties == fp
    def test_site_initialization_invalid_shapes(self, feature_property_fixture_2D, feature_property_fixture_scalar):
    ...
    def test_site_data_generation_1D(self, site_fixture_1d_scalar):
        ...
        n_samples = 100
        covars = np.random.uniform(0,100,n_samples).reshape(1, -1)  # 1D array for single covariate
        baseline_noise = np.random.normal(0, 1, n_samples).reshape(1, -1)  # 1D baseline noise
        ...
        back_transformed_data = (data - site_fixture_1d_scalar.feature_properties.alphas - np.dot(site_fixture_1d_scalar.feature_properties.betas,covars)) / site_fixture_1d_scalar.feature_properties.sigmas
        # remove the site effects
        back_transformed_data = (back_transformed_data-site_fixture_1d_scalar.gamma) / site_fixture_1d_scalar.delta

        np.testing.assert_allclose(back_transformed_data, baseline_noise, rtol=1e-5, err_msg="Back transformed data does not match baseline noise")
    ...
    def test_site_data_generation_2D(self, site_fixture_2d):
        ...
        n_samples = 100
        covars = np.random.uniform(0,100,n_samples*2).reshape(2, -1)  # 2D array for two covariates
        baseline_noise = np.random.normal(0, 1, n_samples*2).reshape(2, -1)  # 2D baseline noise
        ...
        assert isinstance(data, np.ndarray)
        # Test that the output data can be transformed back to the baseline noise by doing the inverse operations
        back_transformed_data = (data - site_fixture_2d.feature_properties.alphas - np.dot(site_fixture_2d.feature_properties.betas,covars)) / site_fixture_2d.feature_properties.sigmas
        # remove the site effects
        back_transformed_data = (back_transformed_data - site_fixture_2d.gamma) / site_fixture_2d.delta
        np.testing.assert_allclose(back_transformed_data, baseline_noise, rtol=1e-5, err_msg="Back transformed data does not match baseline noise")
...
class TestPairwiseComBAT:
    ...
    def test_harmonization_1d(self, ref_site_1d, test_site_1d, baseline_and_covars_1d):
        ...
        moving_data = test_site_1d.generate_site_data(baseline_data, covars.reshape(1, -1))
        ...
        mse_post_combat = mean_squared_error(ref_data.flatten(), combatted_data.flatten())
        mse_pre_combat = mean_squared_error(ref_data.flatten(), moving_data.flatten())
       
        assert mse_post_combat < mse_pre_combat, f"MSE not reduced: before={mse_pre_combat}, after={mse_post_combat}"
...
class TestSaveLoad:
    ...
    def test_save_load_1d(self, tmp_path, ref_site_1d, test_site_1d, baseline_and_covars_1d):
        """Test that harmonization reduces MSE between sites using fixtures"""
        baseline_data, covars = baseline_and_covars_1d
        # Generate synthetic ages for ref and moving
        ref_data = ref_site_1d.generate_site_data(baseline_data, covars.reshape(1, -1))
        ...
        np.testing.assert_allclose(combatted_data,combatted_data_loaded, rtol=1e-5, err_msg="Loaded model predictions do not match original predictions")
    
    def test_save_load_2d(self, tmp_path, ref_site_2d, test_site_2d, baseline_and_covars_2d):
        """Test that harmonization reduces MSE between sites using fixtures"""
        baseline_data, covars = baseline_and_covars_2d
        # Generate synthetic ages for ref and moving
        ref_data = ref_site_2d.generate_site_data(baseline_data, covars)
    ...
    def test_loaded_attributes(self, tmp_path, ref_site_1d, test_site_1d, baseline_and_covars_1d):
        ...
        for attr in dir(combat):
            ...
            if isinstance(val, np.ndarray):
                np.testing.assert_allclose(val, loaded_val, rtol=1e-5, err_msg=f"Attribute {attr} does not match after loading")
            else:
                assert val == loaded_val, f"Attribute {attr} does not match after loading"
