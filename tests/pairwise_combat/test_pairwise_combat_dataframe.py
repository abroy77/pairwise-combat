...
class TestPairwiseComBATDataFrame:
    ...
    def combat_df(self):
        ...
        return PairwiseComBATDataFrame(
            source_site="site_A", target_site="site_B", max_iter=10, tol=1e-4
        )
    ...
    def test_save_load_model(self, combat_df, sample_dataframe):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            original_data = original_transform.select(
                ["voxel_1", "voxel_2", "voxel_3"]
            ).to_numpy()
            loaded_data = loaded_transform.select(
                ["voxel_1", "voxel_2", "voxel_3"]
            ).to_numpy()
    ...
    def test_harmonization_1d_with_sites(self, dataframe_1d):
        ...
        combat = PairwiseComBATDataFrame(
            source_site="test_site", target_site="ref_site", max_iter=30, tol=1e-6
        )
        ...
        ref_data = dataframe_1d.filter(pl.col("site_id") == "ref_site")[
            "voxel_1"
        ].to_numpy()
        test_original = dataframe_1d.filter(pl.col("site_id") == "test_site")[
            "voxel_1"
        ].to_numpy()
        test_harmonized = harmonized_df.filter(pl.col("site_id") == "test_site")[
            "voxel_1"
        ].to_numpy()
        ...
        print(
            f"1D Harmonization: MSE reduced from {mse_pre_combat:.4f} to {mse_post_combat:.4f} "
    ...
    def test_harmonization_2d_with_sites(self, dataframe_2d):
        ...
        ref_data = (
            dataframe_2d.filter(pl.col("site_id") == "ref_site")
            .select(["voxel_1", "voxel_2"])
            .to_numpy()
        )
        test_original = (
            dataframe_2d.filter(pl.col("site_id") == "test_site")
            .select(["voxel_1", "voxel_2"])
            .to_numpy()
        )
        test_harmonized = (
            harmonized_df.filter(pl.col("site_id") == "test_site")
            .select(["voxel_1", "voxel_2"])
            .to_numpy()
        )
        ...
        mse_post_combat = mean_squared_error(
            ref_data.flatten(), test_harmonized.flatten()
        )
        ...
        print(
            f"2D Harmonization: MSE reduced from {mse_pre_combat:.4f} to {mse_post_combat:.4f} "
            f"({improvement:.2f}% improvement)"
        )
